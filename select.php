...
if ($status == false) {
  ...
  while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $view .= h($result['organization'], ENT_QUOTES);
    $view .= '</h2>';
    $view .= '<p class="text-gray-500">';
    $view .= h($result['name'], ENT_QUOTES) . '<br>' . h($result['certification']). '<br>' . 'Email:' . h($result['email']). '<br>' . h($result['website']). '<br><br>' . h($result['details']);
    $view .= '</p></div></div></div>';
  }
}
